<script lang="ts">
	...
	let selected_film_id: number = data.penayangan.film_id;
		...
		<div class="w-full flex-1">
			...
			<Label class="mb-4 space-y-2">
				...
				<Select
					...
					value={form?.audiotorium_id ?? data.penayangan.audiotorium_id}
				/>
			</Label>
			<Label class="mb-4 space-y-2">
				<span>Harga</span>
				<Input
					...
					placeholder="999999"
					size="md"
					value={form?.harga ?? data.penayangan.harga}
					name="harga"
					required
				/>
			</Label>
		...
		<div class="w-full flex-1">
			<Label class="mb-4 space-y-2">
				...
				<Input
					type="date"
					size="md"
					value={form?.tanggal ?? new Date(data.penayangan.tanggal).toISOString().split('T')[0]}
					name="tanggal"
					required
				/>
			</Label>
			<Label class="mb-4 space-y-2">
				...
				<Input
					...
					size="md"
					value={form?.mulai ??
						new Date(data.penayangan.mulai).toLocaleTimeString('en-US', {
							hour: '2-digit',
							minute: '2-digit',
							hour12: false,
						})}
			...
			<Label class="mb-4 space-y-2">
				...
				<Input
					...
					size="md"
					value={form?.selesai ??
						new Date(data.penayangan.selesai).toLocaleTimeString('en-US', {
							hour: '2-digit',
							minute: '2-digit',
							hour12: false,
						})}
